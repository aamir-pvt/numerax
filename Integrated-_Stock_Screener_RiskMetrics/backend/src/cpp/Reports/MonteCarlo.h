...
void calibrateModel(const boost::shared_ptr<ShortRateModel>& model, const std::vector<boost::shared_ptr<BlackCalibrationHelper>>& helpers) {
...
vector<double> MonteCarlo_ZeroCouponBond(int numPath, double maturity, double r0, const vector<double>& DFS) {
	...
	std::vector<boost::shared_ptr<BlackCalibrationHelper>> swaptions;
	...
	for (i = 0; i < numRows; i++) {
		...
		swaptions.push_back(boost::shared_ptr<BlackCalibrationHelper>(new
