	...
	if (type == eFILE)
	{
		...
		if (ifile.is_open())
		{
			...
			if (_originalFile.size() == 0)
				throw CustomError(std::string("No Data in ").append(_file));

			parseHeader();
			parseContent();
		}
		else
			throw CustomError(std::string("Failed to open ").append(_file));
	}
	else
	{
		...
		while (std::getline(stream, line))
			if (line != "")
				_originalFile.push_back(line);
		if (_originalFile.size() == 0)
			throw CustomError(std::string("No Data in pure content"));
...
void Parser::parseContent(void)
{
	...
	for (; it != _originalFile.end(); it++)
	{
		...
		// if value(s) missing
		// cout << row->size() << " " << _header.size();
		if (row->size() != _header.size())
			throw CustomError("corrupted data !");
...
Row &Parser::getRow(unsigned int rowPosition) const
{
	...
	throw CustomError("can't return this row (doesn't exist)");
}
...
{
	if (pos >= _header.size())
		throw CustomError("can't return this header (doesn't exist)");
	return _header[pos];
}
